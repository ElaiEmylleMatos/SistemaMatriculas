      ...
      if(validacep.test(cep)) {
        $.getJSON("https://viacep.com.br/ws/"+ cep +"/json/?callback=?", function(dados) {
          if (!("erro" in dados)) {
            ...
            $("#cep-feedback").html("CEP não encontrado. Preencha os campos manualmente.");
            $("#cep-feedback").show();
          }
        });
      } else {
        $("#cep-feedback").html("Formato de CEP inválido. Por favor, tente novamente.");
        $("#cep-feedback").show();
      }
    } else {
      $("#cep-feedback").hide();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //Quando sai do campo CNPJ aplica a validação e VÊ SE JÁ TEM NO BANCO DE DADOS
    ...
    if (!(c=="")) {
      if (!validarCNPJ(c)) {
        ...
        .done(function(data) {
          if (data == false) {
            $('#cnpjEsc').val("");
            $('#cnpj-feedback').html("CNPJ já cadastrado. Por favor tente outro.");
            $('#cnpj-feedback').show();
          } else {
            $('#cnpj-feedback').hide();
          }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // QUANDO SAI DO CAMPO USUARIO, VÊ SE JÁ TEM NO BANCO DE DADOS
  $('#userEsc').blur(function() {
    var us = $('#userEsc').val();
    if (!(us=="")) {
      $.ajax({
        url: 'userData.php',
      ...
      .done(function(data) {
        if (data == false) {
          $('#userEsc').val("");
          $('#user-feedback').html("Nome de usuário já existente. Por favor escolha outro.");
          $('#user-feedback').show();
        } else {
          $('#user-feedback').hide();
        }
      ...
      if (us.length<3) {
        $('#userEsc').val("");
        $('#user-feedback').html("Nome de usuário precisa ter, no mínimo, 6 caracteres.");
        $('#user-feedback').show();
      } else if (us.length>26) {
        $('#userEsc').val("");
        $('#user-feedback').html("Nome de usuário pode ter no máximo, 26 caracteres.");
        $('#user-feedback').show();
      }
    } else {
  ...
  // QUANDO SAI DO CAMPO SENHA FAZ A VALIDACAO DE TAMANHO
  $('#senhaEsc').blur(function() {
    var senha = $('#senhaEsc').val();
    if (!(senha=="")) {
      if (senha.length<6) {
        $('#senhaEsc').val("");
        $('#senha-feedback').html("A senha precisa ter, no mínimo, 6 caracteres.");
        $('#senha-feedback').show();
      } else if (senha.length>12) {
        $('#senhaEsc').val("");
        $('#senha-feedback').html("A senha precisa pode ter no máximo 12 caracteres.");
        $('#senha-feedback').show();
      } else {
        $('#senha-feedback').hide();
      }
